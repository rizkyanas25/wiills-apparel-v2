      ...
      <div className='absolute w-[100vw] h-[100vh] flex flex-col items-center overflow-x-hidden overflow-y-scroll px-8 py-8 space-y-8'>
        <div className='flex flex-col items-center space-y-8 flex-grow'>
          <div className='text-center space-y-2'>
            ...
            <p className='text-white italic text-xl lg:text-3xl'>
          ...
          {renderLinks([
            ...
            //   icon: <ShopeeIcon className='text-[#E64129] h-[24px] w-[24px]' />,
        ...
        <div className='pb-24'>
          {renderSocialMediaIcons([
            ...
            {
              icon: <Instagram />,
              href: 'https://www.instagram.com/willsapparel.id/',
            },
...
const renderLinks = (
  ...
  <div className='w-full max-w-md space-y-3'>
    {links.map(({ label, icon, href, subtitle, noBlank }) => (
      <Link
        key={label}
        href={href}
...
const renderSocialMediaIcons = (
  socmeds: { icon: React.ReactNode; href: string }[],
) => (
  <div className='flex justify-center space-x-6 text-white font-bold'>
    {socmeds.map(({ icon, href }) => (
      <Link
        ...
        href={href}
        target='_blank'
        className='border border-transparent rounded-full p-2 bg-transparent hover:bg-white hover:bg-white/20 hover:backdrop-blur-lg hover:text-[#E64129] transition-all duration-300 hover:opacity-90 hover:scale-110'
      >
        {icon}
      </Link>
    ))}
